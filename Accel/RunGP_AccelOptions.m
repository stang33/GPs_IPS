...
%                               Y_train consistss of [\dot v_1,..., \dot v_N]
    ...
    elseif ~USE_ACCELERATION
        ...
        %Use subset option with acceleration. TODO
...
multByKInv = @(x) StandardPCG(MultByWholeK, x, PreConInv, CG_ERROR_TOL, CG_ITER_LIMIT);

%Now let's test matrix multiplication.
multMatByKInv = @(X) RunCGOnMatrixInitGuesser(MultByWholeK, X, PreConInv, CG_ERROR_TOL, CG_ITER_LIMIT);
