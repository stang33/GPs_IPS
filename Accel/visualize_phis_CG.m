function learnInfo = visualize_phis_CG(sysInfo,obsInfo,learnInfo,kernel_type, multKInvByMatrix)
...
if learnInfo.order == 1
    phi                  = sysInfo.phi{1}(r);
    [phi_mean,phi_cov]   = phi_pos_CG(r,learnInfo,'E', multKInvByMatrix);
    
elseif strcmp(sysInfo.phi_type, 'EA')
    if kernel_type == 'E'
        phi                  = sysInfo.phi{1}(r);
    else
        phi                  = sysInfo.phi{2}(r);
    end
    [phi_mean,phi_cov]   = phi_pos_CG(r,learnInfo,kernel_type, multKInvByMatrix);
