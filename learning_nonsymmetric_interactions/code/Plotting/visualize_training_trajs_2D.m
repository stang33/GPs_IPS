...
function visualize_training_trajs_2D(sysInfo,obsInfo,solverInfo, learnInfo)
...
if order ==2&&strcmp(learnInfo.name,'FMNS')
    if sysInfo.Noption == 1
        myODE = @(t,y) RHSfn_2nd_ncf_NS1(t,y,N,sysInfo.phi{1},sysInfo.phi{2},sysInfo.phi_type);
    else
        myODE = @(t,y) RHSfn_2nd_ncf_NS(t,y,N,sysInfo.phi{1},sysInfo.phi{2},sysInfo.phi_type);
    end
end
...
color_output                   = construct_color_items(sysInfo.type, train_time_vec(end), test_time_vec);
...
T_loc                          = find(test_time_vec == train_time_vec(end));
