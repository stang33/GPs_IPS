function result = construct_and_compute_traj_NS(sysInfo, obsInfo,  solverInfo,learnInfo,phi_est,ICs)
...
if order ==1&&strcmp(learnInfo.name,'ODNS')
    if sysInfo.Noption == 1
        ...
        myODE1 = @(t,x) RHSfn_NS1(t,x,N,phi_est);
    else
        myODE =  @(t,x) RHSfn_NS(t,x,N,sysInfo.phi{1});
        myODE1 = @(t,x) RHSfn_NS(t,x,N,phi_est);
    end
end
...
if order ==2&&strcmp(learnInfo.name,'CSNS')
    if sysInfo.Noption == 1
        ...
        myODE1 = @(t,y) RHSfn_2nd_NS1(t,y,N,phi_est,sysInfo.phi_type);        
    else
        myODE =  @(t,y) RHSfn_2nd_NS(t,y,N,sysInfo.phi{1},sysInfo.phi_type);
        myODE1 = @(t,y) RHSfn_2nd_NS(t,y,N,phi_est,sysInfo.phi_type);
    end
end
