...
dKs_E = zeros(L*dNs,L*dNs); %derivative w.r.t. logsigma_E
dKo_E = zeros(L*dNs,L*dNs); %derivative w.r.t. logomega_E

if order == 2
    K_A = zeros(L*dNs,L*dNs); % kernelmatrix for phi_A
    dKs_A = zeros(L*dNs,L*dNs); %derivative w.r.t. logsigma_A
    dKo_A = zeros(L*dNs,L*dNs); %derivative w.r.t. logomega_A
end
...
for i = 1:L
    ...
    for j = i+1:L
        ...
        dKs_E((dNs*(j-1)+1):(dNs*j),(dNs*(i-1)+1):(dNs*i)) = dKs_E((dNs*(i-1)+1):(dNs*i),(dNs*(j-1)+1):(dNs*j))';
        dKo_E((dNs*(j-1)+1):(dNs*j),(dNs*(i-1)+1):(dNs*i)) = dKo_E((dNs*(i-1)+1):(dNs*i),(dNs*(j-1)+1):(dNs*j))';
        ...
        if order == 2
            ...
            dKs_A((dNs*(j-1)+1):(dNs*j),(dNs*(i-1)+1):(dNs*i)) = dKs_A((dNs*(i-1)+1):(dNs*i),(dNs*(j-1)+1):(dNs*j))';
            dKo_A((dNs*(j-1)+1):(dNs*j),(dNs*(i-1)+1):(dNs*i)) = dKo_A((dNs*(i-1)+1):(dNs*i),(dNs*(j-1)+1):(dNs*j))';
...
if strcmp('ODS',name)
    if size(hyp,2) >= 3 && ~isnan(hyp(3))
        ...
        K = K + exp(lognoise)^2*eye(dNs*L);
    end
    
elseif size(hyp,2) >= 5 && ~isnan(hyp(5))
        
        lognoise = hyp(5); %noise of Y
        K = K + exp(lognoise)^2*eye(dNs*L);
else
    K = K + learnInfo.jitter*eye(L*dNs);
end
