function [mf] = predict_ode_mcuq(xstar,learnInfo,sysInfo)
...
if order ==1
    ...
    for i=1:N
        ...
        aa(:,i)   = interp1(learnInfo.r,learnInfo.phiE',DD)'/N;   % d x 1
        aa_cov(:,i) = temp*interp1(learnInfo.r,learnInfo.phiE_cov',DD)'/N;
...
if order ==2 
    ...
    for i=1:N
        ...
        aa(:,i)   = temp_position * interp1(learnInfo.r,learnInfo.phiE',DD)'/N + temp_velocity * interp1(learnInfo.r,learnInfo.phiA',DD)'/N;   % 1 x d
        aa_cov(:,i) = sqrt(temp_position.^2 * interp1(learnInfo.r,2*learnInfo.phiE_cov',DD)'.^2/N^2 + temp_velocity.^2*interp1(learnInfo.r,2*learnInfo.phiA_cov',DD)'.^2/N^2);
        aa(:,i)   = aa(:,i) + (rand(d,1)-0.5)*2.*aa_cov(:,i);
    end
