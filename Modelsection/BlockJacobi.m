function [logDetPreCon, PreConInvRaw] = BlockJacobi(learnInfo, LForDecomp, M, rangeOfI, jitter, KE, KA, sigma)
    ...
    numBlocks = M*LForDecomp;
    blockSize = D*N;

    for i = 1 : numBlocks

        Block = K((blockSize*(i-1)+1):blockSize*i,(blockSize*(i-1)+1):blockSize*i);
        PreConInvRaw((blockSize*(i-1)+1):blockSize*i,(blockSize*(i-1)+1):blockSize*i) = pinv(Block);
        logDetPreCon = logDetPreCon + trace(logm(Block));
