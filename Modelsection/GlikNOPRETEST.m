function [fval, dfval,learnInfo] = GlikNOPRETEST(learnInfo, hyp, m, l, CGErrorTol, DerivHVal, M, rangeOfI)
...
PreConInvRaw = ConstructNystPreconNoKOrExtras(learnInfo, LForDecomp, M, rangeOfI, jitter);
...
%FOR USE WITH THIS METHOD! PRECON DOES NOT WORK AS PAPER SAYS.
idPrecon = @(x) x;
...
for i = 1 : l
    ...
    [~,T] = StandardPCGWithT(MultByWholeK, z_i, idPrecon, CGErrorTol, l+1, l);
...
%Make tau.
%tau_star = logDetPreCon / 2 + (n*M*LForDecomp / trueL) * sum(gammas);
tau_star = (n*M*LForDecomp / trueL) * sum(gammas);
