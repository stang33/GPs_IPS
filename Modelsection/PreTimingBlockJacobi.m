...
for k = 1 : nT
    ...
    [fval2, dfval2,~] = GlikSTEPreConSTE52(learnInfo, learnInfo.hyp, mFORGLIK, lFORGLIK, CGErrorTol, HVAL, M, rangeOfI, @BlockJacobi)
    runtimes(1,k) = toc;
    ...
    learnInfo.hyp = [1, 1, 1, 1, 0.5, 1, 1];
    [fval, dfval,~] = GlikSTEPreConSTE52(learnInfo, learnInfo.hyp, mFORGLIK, lFORGLIK, CGErrorTol, HVAL, M, rangeOfI, @BlockJacobi);
    
    Glik_hyp = @(hyp)GlikSTEPreConSTE52(learnInfo, hyp, mFORGLIK, lFORGLIK, CGErrorTol, HVAL, M, rangeOfI, @BlockJacobi);
    ...
    [LOGTEST, PreConInvRaw] = BlockJacobi(learnInfo, LForDecomp, M, rangeOfI, jitter, KE, KA, sigma);
    PreConInv = @(x) PreConInvRaw * x;

%     LOGTEST = LOGTEST
%     TRUETEST = trace(logm(pinv(PreConInvRaw)))
%     TRUETEST = log(det(pinv(PreConInvRaw)))
% 
% 
%     cond(KE + KA + sigma*eye(n*D*M*LForDecomp))
%     cond(PreConInvRaw *(KE + KA + sigma*eye(n*D*M*LForDecomp)))
% 
%     return
