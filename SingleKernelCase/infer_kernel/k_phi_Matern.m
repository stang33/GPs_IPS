function [k, dks, dko] = k_phi_Matern(x1,x2,d,v,hyp,order,name)
...
dks = zeros(n*d,n*d); %derivative of kernel matrix w.r.t. logsigma
dko = zeros(n*d,n*d); %derivative of kernel matrix w.r.t. logomega
                ...
                for k1 = [1:i-1,i+1:n]
                    for k2 = [1:j-1,j+1:n]
                        ...
                        if strcmp('CSF',name)
                            ...
                            dks((d*(i-1)+1):(d*i),(d*(j-1)+1):(d*j)) = dks((d*(i-1)+1):(d*i),(d*(j-1)+1):(d*j))+1/n^2*sigma^2*exp(-omega*abs(norm_rx1(k1,i)-norm_rx2(k2,j)))*rk1x1*rk2x2'*2;
                            dko((d*(i-1)+1):(d*i),(d*(j-1)+1):(d*j)) = dko((d*(i-1)+1):(d*i),(d*(j-1)+1):(d*j))+1/n^2*sigma^2*exp(-omega*abs(norm_rx1(k1,i)-norm_rx2(k2,j)))*rk1x1*rk2x2'*(-omega)*abs(norm_rx1(k1,i)-norm_rx2(k2,j));
                    end
                end
            end
        end
            ...
            for j = 1:n
                for k1 = [1:i-1,i+1:n]
                    for k2 = [1:j-1,j+1:n]
                        ...
                        o_temp_vec = 1/n^2*exp(2*logsigma)*exp(-exp(logomega)*sqrt(3)*abs(norm_rx1(k1,i)-norm_rx2(k2,j)))*rk1x1*rk2x2';
                        o_temp_factor = sqrt(3)* abs(norm_rx1(k1,i)-norm_rx2(k2,j))*exp(logomega);
